...
public:
  ...
  String getFormEnd() override
  {
    String html;
    html.reserve(520);
    html += F("<div class='portal-actions'><p class='portal-help'>Save once you finish editing a batch of settings. Validation errors will stay inside the affected section. Cancel returns to the dashboard without writing any of the unsaved form changes.</p><div class='portal-button-row'><a class='button-link ghost' href='/'>Cancel</a>");
    appendLogoutAction(html);
    html += F("<a class='button-link danger' href='/reboot'>Reboot</a><button type='submit'>Save Configuration</button></div></div></form>");
    return html;
  }
